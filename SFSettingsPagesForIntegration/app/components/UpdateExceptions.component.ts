...
@Component({
    ...
    templateUrl: 'partials/pages/UpdateExceptions.component.html',
    inputs: ['userName', 'FilterDays', 'AllResponses'],
    providers: [ExceptionsService, JSONP_PROVIDERS]
})

//This component is used to display the exception which were raised during update/ insert note in CC
export class UpdateExceptions implements OnInit {
    userName: string;
    accesToken: string;
    ...
    GetExceptions() {
        var that = this;
        this.exception.GetExceptions(this.userName, this.IntegrationType, 'Update', this.FilterDays).then(
            function (data) {
    ...
    this.ExceptionRecordsListBackUp = null;
        ...
        this.exception.ArchiveExceptions(myJsonString, this.userName, this.IntegrationType, 'Update', 'Archive').then(function (data) {
            that.GetExceptions();
        }).catch(function (data) {
            that.GetExceptions();
        });
    ...
    //Enabling or  disabling  Re trigger  in CCFD 
